...
Float_t z_off=4.5;//to the physical end of array from the target (-20 to si start)
...
void Monitors::Begin(TTree *tree)
{
   ...
   //array loop
   hecalVz = new TH2F("hecalVz","E vs. Z;Z (cm);E (MeV)",400,-50,0,400,0,20);
   hecalVzR = new TH2F("hecalVzR","E vs. Z gated;Z (cm);E (MeV)",700,-55,0,750,0,3000);
   ...
   helum4=new TH1F("helum4", "downstream dE; E (channels); count / 10 channels", 800, 0, 8000);
   
   helum4Entry = new TH2F("helum4Entry", "downstream dE vs time; time [min]; dE channel", 500, 350, 900, 500, 0, 9000);
   helum4D = new TH1F("helum4D", "downstream dE (deuteron); time [min]; count / 1 min ", 550, 350, 900);
   helum4C = new TH1F("helum4C", "downstream dE (carbon) ; time [min]; count / 1 min",  550,  350, 900);
   ...
   //energy spectrum
   hexC = new TH1F("hexC","excitation spectrum; E [MeV] ; Count / 50 keV",240,-2,10);
...
void Monitors::Terminate()
{
   ...
   cCanvas->cd(); cCanvas->Divide(2,3);
   
   gStyle->SetOptStat("neiou");
   
   cCanvas->cd(1); helum4Entry->Draw("colz");
   cCanvas->cd(2); helum4->Draw("colz");
   
   cCanvas->cd(3); helum4D->Draw("");
   cCanvas->cd(4); hecalVz->Draw("colz");
   
   cCanvas->cd(5); helum4C->Draw("");
   cCanvas->cd(6); hexC->Draw("");
