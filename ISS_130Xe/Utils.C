...
void eVx(void) {
  TCanvas *ceVx = new TCanvas("ceVx","EVX",1600, 1200);
...
void elum(void) {
  TCanvas *celum = new TCanvas("celum","ELUM",1000,500);
  celum->Clear();
...
void ecalVz(void) {
  TCanvas *cecalVz = new TCanvas("cevalVz","ECALVZ",1200,800);
  cecalVz->Clear(); cecalVz->Divide(1,1);
  gStyle->SetOptStat("neiou");
...
void fitExcite(int bgEst = 10, double peakThreshold = 0.1, TH1 * hist = hexC){
