{
   ...
   */
   ...
   for( int i = 0 ; i < 6 ; i++){
      for(int j = 0; j < 4 ; j++){
         ...
         if( i >= 3){
            ...
            gate.Form("TMath::Abs(energy_t)<20 && det == %d && ttt > 0", i + 6*j);
            
            if( i == 3 && (j == 1 || j == 3) ) gate.Form("TMath::Abs(energy_t)<20 && det == %d && ttt > -100", i + 6*j);
   ...
   TH2F * hb = new TH2F("hb", "hb" , 400, -2000, 1500, 400, -600, -200);
   hb->SetXTitle("Ex [a.u.]");
   for( int i = 0 ; i < 6 ; i++){
      for(int j = 0; j < 4 ; j++){
   
         expression.Form("x[%d]: energy >> + hb", i + 6*j);
         TString gate;
         if( i >= 3){
            gate.Form("TMath::Abs(energy_t)<20 && det == %d && ttt< 0", i + 6*j);
            ...
            gate.Form("TMath::Abs(energy_t)<20 && det == %d && TMath::Abs(ttt)>1000");
         }        
         tree->Draw(expression, gate , "");
      }
   }
